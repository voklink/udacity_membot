...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {
                            // _nodes.emplace_back(new GraphNode(id));                  // Adding a new GraphNodePtr raw with ID via "new"
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));       // Adding a new UNQPtr GarphNode
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        // Here, "->" gets you access of the ACTUAL OBJECT
        if ((*it)->GetNumberOfParents() == 0)
        {

            if (rootNode == nullptr)
            {
                // You need to get the HANDLE from the current node.
